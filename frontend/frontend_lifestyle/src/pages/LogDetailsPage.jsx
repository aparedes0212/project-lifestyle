...
export default function LogDetailsPage() {
  ...
  // miles per 1 "unit" (e.g., 400m ‚âà 0.248548 mi)
  ...
  return (
    <div>
      <div style={{ marginBottom: 12 }}>
        <Link to="/cardio" style={{ textDecoration: "none" }}>‚Üê Back</Link>
      </div>
      <Card title={`Log #${id}`} action={null}>
        ...
        {!loading && !error && data && (
          <>
            ...
            {unitTypeLower === "time" ? (
              <div style={{ marginBottom: 8 }}>
                <table style={goalsTableStyle}>
                  ...
                  <tbody>
                    <tr>
                      <td style={goalsTableCellStyle}>{formatGoalLabel(goalTableData?.total.unitValue) ?? "ó"}</td>
                      <td style={goalsTableCellStyle}>{goalTableData?.unitLabel ?? "ó"}</td>
                      <td style={goalsTableCellStyle}>{formatGoalLabel(goalTableData?.total.mph) ?? "ó"}</td>
                      <td style={goalsTableCellStyle}>{formatGoalLabel(goalTableData?.total.miles) ?? "ó"}</td>
                    </tr>
                    <tr>
                      <td style={goalsTableCellStyle}>{formatGoalLabel(goalTableData?.goal.unitValue) ?? "ó"}</td>
                      <td style={goalsTableCellStyle}>{goalTableData?.unitLabel ?? "ó"}</td>
                      <td style={goalsTableCellStyle}>{formatGoalLabel(goalTableData?.goal.mph) ?? "ó"}</td>
                      <td style={goalsTableCellStyle}>{formatGoalLabel(goalTableData?.goal.miles) ?? "ó"}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ) : (
              <Row left="Goal" right={data.goal ?? "ó"} />
            )}
                ...
                return (
                  <div style={{ textAlign: "right", display: "grid", gap: 6 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8 }}>
                      <div>
                        <span style={{ opacity: 0.8 }}>Max:</span> {maxVal ?? "‚Äî"}
                        {avgVal != null && (
                          <span>
                            {"  |  "}
              ...
              <Row
                ...
                right={
                  <div style={{ textAlign: "right" }}>
                    <div>{goalTimeGoal != null ? formatMinutesValue(goalTimeGoal) : "‚Äî"}</div>
                  </div>
                }
              />
            )}
            ...
            <Row left="Avg MPH" right={data.avg_mph ?? "‚Äî"} />
            <Row left="Minutes Elapsed" right={data.minutes_elapsed ?? "‚Äî"} />
            <Row
              left="Rest Timer"
              right={
                <span
            ...
            <form onSubmit={submit}>
              <div style={{ display: "grid", gap: 8, gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))" }}>
                ...
                <label>
                  ...
                  <select value={unitId} onChange={(e) => setUnitId(e.target.value)} disabled={unitsApi.loading || !distanceUnits.length}>
                    {unitsApi.loading && <option value="">Loading‚Ä¶</option>}
                ...
                <label>
                  ...
                  <select value={tmSync} onChange={(e) => setTmSync(e.target.value)}>
                    <option value="run_to_tm">Run time ‚Üí TM</option>
                    <option value="tm_to_run">TM ‚Üí Run time</option>
              ...
              <div style={{ marginTop: 8 }}>
                <button type="submit" style={btnStyle} disabled={saving || unitsApi.loading}>{saving ? "Saving‚Ä¶" : (editingId ? "Save interval" : "Add interval")}</button>
